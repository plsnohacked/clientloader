...
local function main()
    ...
    Data = {
        Comparison = {
            ...
            rad = {
                enabled = function()
                    return service.Players.LocalPlayer.Character 
                        and service.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") 
                        and service.Players.LocalPlayer.Character.HumanoidRootPart:IsA("BasePart")
                end,
                func = function(argString)
        ...
        Specific = {
            ...
            loadedmodules = {
                enabled = function() return env.getloadedmodules ~= nil end,
                func = function()
        ...
        Default = {
            enabled = true,
            func = function(argString,caseSensitive)
    ...
    -- Adapt the original interface with wrapper functions
    Comparison = setmetatable({}, {
        __index = function(t, k)
            local filter = Explorer.SearchFilters.Data.Comparison[k]
            if filter and (type(filter.enabled) == "function" and filter.enabled() or filter.enabled == true) then
                return filter.func
            end
            return nil
        end
    ...
    Specific = setmetatable({}, {
        __index = function(t, k)
            local filter = Explorer.SearchFilters.Data.Specific[k]
            if filter and (type(filter.enabled) == "function" and filter.enabled() or filter.enabled == true) then
                return filter.func
            end
            return nil
    ...
    Default = function(argString, caseSensitive)
        if Explorer.SearchFilters.Data.Default.enabled then
            return Explorer.SearchFilters.Data.Default.func(argString, caseSensitive)
        end
    end,
    
    SpecificDefault = function(n)
        if Explorer.SearchFilters.Data.SpecificDefault.enabled then
            return Explorer.SearchFilters.Data.SpecificDefault.func(n)
        end
    end
}
...
Properties.GetExpandedProps = function(prop)
    local PROPERTY_CONFIGS = {
        ...
        CFrame = {
            {suffix = ".Position", type = "Vector3"},
            {suffix = ".RightVector", type = "Vector3"},
            {suffix = ".UpVector", type = "Vector3"},
            {suffix = ".LookVector", type = "Vector3"}
        },
        UDim = {
            {suffix = ".Scale", type = "float"},
            {suffix = ".Offset", type = "int"}
        },
        UDim2 = {
            {suffix = ".X", type = "UDim"},
            {suffix = ".Y", type = "UDim"}
        ...
        Rect = {
            {suffix = ".Min.X", type = "float", name = "X0"},
            {suffix = ".Min.Y", type = "float", name = "Y0"},
            {suffix = ".Max.X", type = "float", name = "X1"},
            {suffix = ".Max.Y", type = "float", name = "Y1"}
        },
        PhysicalProperties = {
            {suffix = ".Density", type = "float"},
            {suffix = ".Elasticity", type = "float"},
            {suffix = ".ElasticityWeight", type = "float"},
            {suffix = ".Friction", type = "float"},
            {suffix = ".FrictionWeight", type = "float"}
        },
        Ray = {
            {suffix = ".Origin", type = "Vector3"},
            {suffix = ".Direction", type = "Vector3"}
        },
        NumberRange = {
            {suffix = ".Min", type = "float"},
            {suffix = ".Max", type = "float"}
        },
        Faces = {
            {suffix = ".Back", type = "bool"},
            {suffix = ".Bottom", type = "bool"},
            {suffix = ".Front", type = "bool"},
            {suffix = ".Left", type = "bool"},
            {suffix = ".Right", type = "bool"},
            {suffix = ".Top", type = "bool"}
        },
        Axes = {
            {suffix = ".X", type = "bool"},
            {suffix = ".Y", type = "bool"},
    ...
    if prop.Name == "SoundId" and prop.Class == "Sound" then
        return {Properties.SoundPreviewProp}
    end

    -- Use the lookup table if exists
    local config = PROPERTY_CONFIGS[typeName]
    if config then
        for i, entry in ipairs(config) do
            result[i] = Properties.MakeSubProp(
                prop,
                entry.suffix,
                {Name = entry.type},
                entry.name
...
Properties.DisplayBrickColorEditor = function(prop,entryIndex,col)
    ...
    if not editor then
        ...
        editor.OnMoreColors:Connect(function()
            editor:Close()
            local colProp
            -- Special case: If editing BasePart.BrickColor, switch to BasePart.Color
            if editor.CurrentProp and editor.CurrentProp.Class == "BasePart" and editor.CurrentProp.Name == "BrickColor" then
                for i,v in pairs(API.Classes.BasePart.Properties) do
                    if v.Name == "Color" then
                        colProp = v
                        -- Convert BrickColor to Color3 for the color editor
                        local color3Value = editor.SavedColor.Color
                        Properties.DisplayColorEditor(colProp, color3Value)
                        return
                    end
                end
            end
...
local function main()
    ...
    -- Helper function to safely decompile
    local function safeDecompile(scr)
        local success, source = pcall(env.decompile or function() end, scr)
        if not success or not source then
            return "-- DEX - Source failed to decompile", nil
        end
        return source:gsub("\0", "\\0"), scr  -- Fix null character issue
    end

    ScriptViewer.ViewScript = function(scr)
        local source, newPrevious = safeDecompile(scr)
        codeFrame:SetText(source)
        PreviousScr = newPrevious
        window:Show()
    end

    -- Function dumping utilities
    local function createFunctionDumper()
        ...
        local function addToDump(str, indentation, new_line)
            dump = dump .. ("%s%s%s"):format(
                string.rep("    ", indentation or 0),
                tostring(str),
                new_line == false and "" or "\n"
            )
        end

        local function getFunctionName(func)
            local info = getinfo(func)
            return info and info.name ~= "" and info.name or "Unknown Name"
        end

        local function dumpTable(input, indent, index)
            indent = indent < 0 and 0 or indent
            addToDump(("%s [%s] %s"):format(
                tostring(index),
            ...
            for idx, value in pairs(input) do
                ...
                elseif type(value) == "table" then
                    if not data_base[value] then
                        ...
                        addToDump(("%d [table]:"):format(count), indent)
                        dumpTable(value, indent + 1, idx)
                    else
                        addToDump(("%d [table] (Recursive table detected)"):format(count), indent)
                    end
                else
                    addToDump(("%d [%s] = %s"):format(
                        count,
        ...
        local function dumpFunction(input, indent)
            local funcName = getFunctionName(input)
            addToDump(("\nFunction Dump: %s"):format(funcName), indent)
            
            -- Dump upvalues
            addToDump(("\nFunction Upvalues: %s"):format(funcName), indent)
            for idx, upvalue in pairs(getupvalues(input)) do
                ...
                elseif type(upvalue) == "table" then
                    if not data_base[upvalue] then
                        ...
                        addToDump(("%d [table]:"):format(idx), indent + 1)
                        dumpTable(upvalue, indent + 2, idx)
                    else
                        addToDump(("%d [table] (Recursive table detected)"):format(idx), indent + 1)
                    end
                else
                    addToDump(("%d [%s] = %s"):format(
                        idx,
            ...
            -- Dump constants
            addToDump(("\nFunction Constants: %s"):format(funcName), indent)
            for idx, constant in pairs(getconstants(input)) do
                ...
                elseif type(constant) == "table" then
                    if not data_base[constant] then
                        ...
                        addToDump(("%d [table]:"):format(idx), indent + 1)
                        dumpTable(constant, indent + 2, idx)
                    else
                        addToDump(("%d [table] (Recursive table detected)"):format(idx), indent + 1)
                    end
                else
        ...
        return function(scriptInstance)
            ...
            for _, func in pairs(getgc()) do
                if typeof(func) == "function" and getfenv(func).script == scriptInstance then
                    dumpFunction(func, 0)
                    addToDump("\n" .. ("="):rep(100), 0, false)
                end
            end
    ...
    ScriptViewer.Init = function()
        ...
        save.MouseButton1Click:Connect(function()
            ...
            local filename = string.format(
                "Place_%s_Script_%d.txt",
                game["Run Service"].Parent.PlaceId,
                os.time()
            )
        ...
        local dumpFunctions = createFunctionDumper()
        dumpbtn.MouseButton1Click:Connect(function()
            if PreviousScr then
                pcall(function()
                    local dumpResult = dumpFunctions(PreviousScr)
                    if dumpResult then
                        codeFrame:SetText(codeFrame:GetText() .. dumpResult)
                    end
                end)
            end
        end)
...
local function renderSide(side, noTween)
    ...
    -- Disconnect all resize connections
    for i, v in pairs(side.ResizeCons) do v:Disconnect() end
    side.ResizeCons = {}
    side.Resizing = nil

    -- Get existing resizers instead of destroying them all
    ...
    local resizerIndex = 1  -- Track which existing resizer we're using
    ...
    for i, v in pairs(side.Windows) do
        ...
        if not isEnd then
            local newTemplate
            -- Reuse existing resizer if available
            if existingResizers[resizerIndex] then
                newTemplate = existingResizers[resizerIndex]
                resizerIndex = resizerIndex + 1
            else
                -- Create new resizer only if needed
    ...
    -- Destroy any leftover resizers that weren't reused
    ...
    if align == nil and window.ClosedSide then -- Regular open
        ...
        -- Improved window opening animation and handling
        local initialSize = UDim2.new(0, window.SizeX, 0, 20)
        local targetSize = UDim2.new(0, window.SizeX, 0, size or window.SizeY)
        
        -- Store original properties
        window.OriginalSize = window.OriginalSize or targetSize
        window.SizeY = size or window.SizeY
        
        -- Prepare window for animation
        window.GuiElems.Main.Size = initialSize
        window.GuiElems.Main.Visible = true
        
        -- Create smooth tween
        local ti = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        window:StopTweens()
        
        -- Add optional fade-in effect
        if window.GuiElems.Main:FindFirstChild("BackgroundTransparency") then
            ...
            window:DoTween(window.GuiElems.Main, ti, {Size = targetSize})
        end

        -- Add to visible windows and update
        table.insert(visibleWindows, 1, window)
        updateWindows()
    end
end
...
-- Improved tab handling with constants
local TAB_REPLACEMENT_IN = (" %s%s "):format(tabSub, tabSub)
local TAB_REPLACEMENT_OUT = "\t"

funcs.ConvertText = function(self, text, toEditor)
    if toEditor then
        return text:gsub("\t", TAB_REPLACEMENT_IN)
    else
        return text:gsub(TAB_REPLACEMENT_IN, TAB_REPLACEMENT_OUT)
    end
end

-- Improved GetText with direct tab handling
funcs.GetText = function(self)
    local result = {}
    local lines = self.Lines
    
    for i = 1, #lines do
        -- Convert tabs in each line directly
        result[i] = lines[i]:gsub(TAB_REPLACEMENT_IN, TAB_REPLACEMENT_OUT)
    end
...
funcs.SetText = function(self, txt)
    txt = self:ConvertText(txt, true) -- Tab Convert to editor format
    local lines = self.Lines
    table.clear(lines)
    local count = 1
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="V2.0",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
    ...
    -- Handle app name conflict by either:
    -- 1. Returning false/nil to indicate failure
    -- 2. Appending a number to make the name unique
    -- 3. Replacing the existing app
    if Main.MenuApps[data.Name] then 
        -- Option 1: Simply return (current behavior)
        -- return false 
        
        -- Option 2: Make unique name by appending number
        -- local counter = 1
        -- while Main.MenuApps[data.Name..tostring(counter)] do
        --     counter = counter + 1
        -- end
        -- data.Name = data.Name..tostring(counter)
        
        -- Option 3: Close/remove existing app
        -- Main.MenuApps[data.Name].Window:Close()
        -- Main.MenuApps[data.Name] = nil
        
        return -- Current behavior (just return)
    end
    ...
    -- Icon handling (existing good implementation)
...
Main.SetupFilesystem = function()
    if type(env) ~= "table" or not env.writefile or not env.makefolder then
        return false, "Required filesystem functions are missing"
    end
    ...
    local ok, err = createDirectory(DIRECTORY_STRUCTURE.root)
    if not ok then return false, err end

    for _, dir in ipairs(DIRECTORY_STRUCTURE.subdirectories) do
        local fullPath = DIRECTORY_STRUCTURE.root .. "/" .. dir
        ok, err = createDirectory(fullPath)
        if not ok then return false, err end
    end

    local criticalDirs = { "resources", "plugins" }
    for _, dir in ipairs(criticalDirs) do
        local fullPath = DIRECTORY_STRUCTURE.root .. "/" .. dir
    end

    return true, "Filesystem initialized successfully"
end
...
Main.SaveExternalDependencies = function()
    if not Main.Elevated or not env.writefile then
        return false, "Insufficient permissions or missing filesystem API"
    end

    if not Main.ClientVersion or not Main.RobloxVersion then
        return false, "Missing version information"
    end

    if not Main.RawAPI or not Main.RawRMD then
        return false, "Missing API/RMD data"
    end

    local DEPENDENCY_FILES = {
        {
            path = "dex/deps_version.dat",
        ...
        {
            path = "dex/rbx_api.dat",
            content = Main.RawAPI,
            critical = true
        },
        {
            path = "dex/rbx_rmd.dat",
            content = Main.RawRMD,
            critical = true
        }
    }
    ...
    if Main.LocalDepsUpToDate() then
        return true, "Dependencies already up-to-date"
    end

    if not pcall(env.makefolder, "dex") then
        return false, "Failed to create dex directory"
    end

    for _, file in ipairs(DEPENDENCY_FILES) do
        local success, err = pcall(env.writefile, file.path, file.content)
        
        if not success then
            if file.critical then
                return false, string.format("Failed to write critical file %s: %s", file.path, err)
            end
            warn(string.format("Failed to write %s: %s", file.path, err))
        end
    end

    for _, file in ipairs(DEPENDENCY_FILES) do
        if file.critical and not pcall(env.readfile, file.path) then
            return false, string.format("Verification failed for %s", file.path)
        end
    end

    return true, "Dependencies successfully updated"
end

if Main.Elevated and env.writefile and not Main.LocalDepsUpToDate() then
    local success, err = Main.SaveExternalDependencies()
    if not success then
        warn("Dependency save failed:", err)
    end
end
